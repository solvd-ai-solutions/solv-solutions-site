    ...
    >
      ...
      <div className="grid gap-1.5">
        {payload.map((item, index) => {
          ...
          const itemConfig = getPayloadConfigFromPayload(config, item, key);
          const indicatorColor = color || (typeof item.payload === 'object' && item.payload && 'fill' in item.payload ? (item.payload as { fill?: string }).fill : undefined) || item.color;
...
function ChartLegendContent({
  ...
  return (
    ...
    >
      {payload.map((item) => {
        const typedItem = item as { dataKey?: string; value?: string | number; [key: string]: unknown };
          ...
          >
            {itemConfig?.icon && !hideIcon ? (
              ...
              <div
                ...
                style={{
                  backgroundColor: typedItem.color as string | undefined,
                }}
              />
            )}
            {itemConfig?.label}
